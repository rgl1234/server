...
class RepairMimeTypes implements IRepairStep {
	...
	private function updateMimetypes($updatedMimetypes) {
		...
		$query->select('id')
			->from('mimetypes')
			->where($query->expr()->eq('mimetype', $query->createParameter('mimetype')));
		...
		foreach ($updatedMimetypes as $extension => $mimetype) {
			...
			$query->setParameter('mimetype', $mimetype, IQueryBuilder::PARAM_INT);
			$result = $query->execute();
			...
			if (!$mimetypeId) {
				// insert mimetype
				$insert->setParameter('mimetype', $mimetype, IQueryBuilder::PARAM_INT);
				$insert->execute();
